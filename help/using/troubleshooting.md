...
* Accertati che tutte le API richieste - Asset Compute, Adobe [!DNL I/O Events], Gestione eventi e Runtime - siano incluse in Adobe [!DNL `I/O Project`] e Workspace per evitare errori di richiesta `/register` o `/process`.

## Accesso ai problemi tramite Adobe [!DNL aio-cli] {#login-via-aio-cli}

In caso di problemi durante l&#39;accesso a [!DNL Adobe Developer Console] [tramite Adobe [!DNL aio-cli]](https://developer.adobe.com/app-builder/docs/get_started/app_builder_get_started/first-app#3-signing-in-from-cli), aggiungere manualmente le credenziali necessarie per sviluppare, testare e distribuire l&#39;applicazione personalizzata:
...
1. Aggiungi le credenziali di Adobe [!DNL I/O Runtime]. Ottieni le credenziali di Adobe [!DNL I/O Runtime] dal JSON scaricato. Credenziali in `project.workspace.services.runtime`. Aggiungere le credenziali di runtime [!DNL Adobe I/O] nelle variabili `AIO_runtime_XXX`:
